...
int main(int argc, char const *argv[])
{
    ...
    for (t = dt; t <= 200.; t+=dt)
    {
        ...
        if (j3>20)
        {
            ...
            r1 = fabs(x[i1])+fabs(x[i2])+fabs(v[i1])+fabs(v[i2]);
        if (r1>1e-20)
        {
            i1=i1-5;
            i2=i2+5;
        ...
        if (j>100)
        {
            ...
            for (i = 1; i <= N; i++)
            {
                ...
                r2 = 0.5*m[i]*(v[i]*v[i])+0.5*kmola[i]*(r1);
                
                ET+=r2;

                fprintf(fil1,"%15.12g  %10d  %15.12E\n",t,i,r2);
            }
            r1 = fabs(x[i1])+fabs(x[i2])+fabs(v[i1])+fabs(v[i2]);
            j = 0.;
            fprintf(fil2,"%15.12g   %15.12g    %15.12g   %15d \n",t,fabs(1-E0/ET),r1,abs(i1-i2));
        }
